...
$all_user_role = [];
$all_prod_db = [];
$myque = "SELECT * FROM `user_role` WHERE `is_active`='1'";
$conn_que = mysqli_query($conn, $myque);
while ($row = mysqli_fetch_array($conn_que)) {
    $all_user_role[$row['user_id']] = $row['role'];
    $all_prod_db[$row['user_id']] = '';
}

$myque_prod = "SELECT * FROM `user_prod` WHERE `is_active`='1'";
$conn_que_prod = mysqli_query($conn, $myque_prod);
while ($row = mysqli_fetch_array($conn_que_prod)) {
    $all_prod_db[$row['user_id']] .= $row['product'] . ", ";
}
//var_dump($all_prod_db);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["user_id"])) {
        ...
        if ($_SESSION["login_user"] == $user_id && isset($_SESSION["super_admin"])) {
            $_SESSION['double_create_error'] = 'You Are Super-Admin!';
...
<body>
    ...
    <main class="mt-5 pt-3">
        <div class="container">
            ...
            <table class="table table-striped table-bordered table-hover mt-2" id="userTable">
                <thead>
                    <tr>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                ...
                                echo $all_prod_db[$prod_user_id];
                                ?>

                            </td>
                            <td>
                                <?php
                                if (isset($all_user_role[$prod_user_id])) {
                                    echo $all_user_role[$prod_user_id];
                                } else {
                                    echo "<b>Role Removed!</b>";
                                }
